...
async def provide_insights(request: Request):
    ...
    basestring = f"v0:{slack_request_ts}:{request_body}".encode("utf-8")

    sig = (
        "v0="
        + hmac.new(
            bytes(slack_signing_secret, "utf8"), basestring, hashlib.sha256
        ).hexdigest()
    )

    if not hmac.compare_digest(sig, slack_signature):
